...
export default function InkReveal({
  ...
  // When an external element is the event source, listen there instead of the canvas.
  // This lets the canvas sit at a low z-index while mouse tracking works through
  // overlapping content.
  useEffect(() => {
    if (!eventSourceRef) return;
    const el = eventSourceRef.current;
    if (!el) return;
  ...
  }, [eventSourceRef, stampAlong, startLoop]);
  ...
  const selfHandlers = !eventSourceRef
    ? {
        onMouseEnter: (e) => {
  ...
  return (
    <canvas
      ...
      style={{
        ...
        inset: 0,
        zIndex: 1,
        pointerEvents: eventSourceRef ? "none" : "auto",
        cursor: eventSourceRef ? "auto" : "none",
        ...style,
      }}
      {...selfHandlers}
    />
